...
const Circulars = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (response.ok) {
          ...
          const circularsDataWithAbsoluteUrls = data.map(item => ({
            ...item,
            file: `http://localhost:5000/${item.file}`,
          }));
          setCircularsData(circularsDataWithAbsoluteUrls);
        } else {
          console.error('Failed to fetch Circulars data');
        }
      } catch (error) {
  ...
  const handleSearchSubmit = () => {
    const foundItems = circularsData.filter(item =>
      item.title.toLowerCase().includes(searchQuery.toLowerCase())
    );
                    ...
                    <div className="modal-dialog">
                      <div className="modal-content">
                        <div className="modal-header">
                          <h5 className="modal-title">Circular</h5>
                          <button
                            type="button"
                            className="btn-close"
                            onClick={closeModal}
