...
const GradationList = () => {
  const [gradationListData, setGradationListData] = useState([]);
    ...
    const fetchData = async () => {
      try {
        ...
        if (response.ok) {
          ...
          const gradationListDataWithAbsoluteUrls = data.map(item => ({
            ...item,
            file: `http://localhost:5000/${item.file}`,
          }));
          setGradationListData(gradationListDataWithAbsoluteUrls);
        } else {
          console.error('Failed to fetch Gradation List data');
        }
      } catch (error) {
  ...
  const filteredData = gradationListData.filter(item =>
  ...
  const handleSearchSubmit = () => {
    const foundItems = gradationListData.filter(item =>
      item.title.toLowerCase().includes(searchQuery.toLowerCase())
    );
                    ...
                    <div className="modal-dialog">
                      <div className="modal-content">
                        <div className="modal-header">
                          <h5 className="modal-title">Gradation List</h5>
                          <button
                            type="button"
                            className="btn-close"
                            onClick={closeModal}
