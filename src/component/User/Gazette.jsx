...
const GazetteWeb = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (response.ok) {
          ...
          const gazetteDataWithAbsoluteUrls = data.map(item => ({
            ...item,
            file: `http://localhost:5000/${item.file}`,
          }));
          setGazetteData(gazetteDataWithAbsoluteUrls);
        } else {
          console.error('Failed to fetch Gazette data');
        }
      } catch (error) {
  ...
  const filteredData = gazetteData.filter(item =>
  ...
  const handleSearchSubmit = () => {
    const foundItems = gazetteData.filter(item =>
      item.title.toLowerCase().includes(searchQuery.toLowerCase())
    );
                    ...
                    <div className="modal-dialog">
                      <div className="modal-content">
                        <div className="modal-header">
                          <h5 className="modal-title">Gazette</h5>
                          <button
                            type="button"
                            className="btn-close"
                            onClick={closeModal}
