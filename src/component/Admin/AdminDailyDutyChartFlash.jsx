...
const AdminDailyDutyChartFlash = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch("http://localhost:5000/dailydutychartflash");
        if (response.ok) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/dailydutychartflash/${id}`, {
        method: 'DELETE',
      });
      if (response.ok) {
  ...
  const handleUpdate = (item) => {
    navigate('/admindailydutychartflashform', { state: { dailydutychartflashToUpdate: item } });
  };
  ...
  const totalPages = Math.ceil(DailyDutyChartFlash.length / rowsToShow);
  ...
  const slicedDailyDutyChartFlash = DailyDutyChartFlash.slice(startIndex, endIndex);
            ...
            <div className="col-xl-12">
              <div className="card ">
                ...
                <form className="d-flex row m-3 align-items-center">
                  <div className="col-xl-3"> <h4>DailyDutyChartFlash </h4></div>
                  ...
                  <div className="btn-box col-xl-3">
                    <a href="/admindailydutychartflashform" className="btn btn-primary"><i className="fa-light fa-plus"></i> Add New</a>
                  </div>
                </form>
                <div className="card-body">
                  <table className="table table-striped">
                    ...
                    <tbody>
                      <tr></tr>
                      {slicedDailyDutyChartFlash.map((item, index) => (
                        <tr key={index}>
                          <td>{index + 1}</td>
                          <td>{item.name}</td>
...
export default AdminDailyDutyChartFlash
