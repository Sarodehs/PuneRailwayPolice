...
const AdminCirculars = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/circulars/${id}`, {
        method: 'DELETE',
      });
      if (response.ok) {
  ...
  const handleUpdate = (item) => {
    navigate('/admincircularsform', { state: {circularsToUpdate: item } });
  };
  ...
  const totalPages = Math.ceil(Circulars.length / rowsToShow);
  ...
  const slicedCirculars = Circulars.slice(startIndex, endIndex);
            ...
            <div className="col-xl-12">
              <div className="card ">
                ...
                <form className="d-flex row m-3 align-items-center">
                  <div className="col-xl-3"> <h4>Circulars </h4></div>
                  ...
                  <div className="btn-box col-xl-3">
                    <a href="/admincircularsform" className="btn btn-primary"><i className="fa-light fa-plus"></i> Add New</a>
                  </div>
                </form>
                <div className="card-body admintablesroll">
                  <table className="table table-striped">
                    ...
                    <tbody>
                      <tr></tr>
                      {slicedCirculars.map((item, index) => (
                        <tr key={index}>
                          <td>{index + 1}</td>
                          <td>{item.title}</td>
