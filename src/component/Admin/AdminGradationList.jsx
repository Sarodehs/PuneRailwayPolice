...
const AdminGradationList = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/gradationlist/${id}`, {
        method: 'DELETE',
      });
      if (response.ok) {
  ...
  const handleUpdate = (item) => {
    navigate('/admingradationlistform', { state: { gradationListToUpdate: item } });
  };
  ...
  const totalPages = Math.ceil(GradationList.length / rowsToShow);
  ...
  const slicedGradationList = GradationList.slice(startIndex, endIndex);
            ...
            <div className="col-xl-12">
              <div className="card ">
                ...
                <form className="d-flex row m-3 align-items-center">
                  <div className="col-xl-3"> <h4>GradationList </h4></div>
                  ...
                  <div className="btn-box col-xl-3">
                    <a href="/admingradationlistform" className="btn btn-primary"><i className="fa-light fa-plus"></i> Add New</a>
                  </div>
                </form>
                <div className="card-body">
                  <table className="table table-striped">
                    ...
                    <tbody>
                      <tr></tr>
                      {slicedGradationList.map((item, index) => (
                        <tr key={index}>
                          <td>{index + 1}</td>
                          <td>{item.title}</td>
...
export default AdminGradationList
