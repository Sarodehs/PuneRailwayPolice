...
const AdminWomenHelpDesk = () => {
 ...
 useEffect(() => {
  console.log('Fetched data:', WomenHelpDesk);
}, [WomenHelpDesk]);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/womenHelpDesk/${id}`, {
        method: 'DELETE',
      });
     
      if (response.ok) {
        // Remove the deleted item from the state
        setWomenHelpDesk(WomenHelpDesk.filter(item => item._id !== id));
  ...
  const handleUpdate = (item) => {
    navigate('/adminwomenhelpdeskform', { state: { womenhelpdeskToUpdate: item } });
  };
  ...
  const totalPages = Math.ceil(WomenHelpDesk.length / rowsToShow);
  ...
  const slicedWomenHelpDesk = WomenHelpDesk.slice(startIndex, endIndex);
            ...
            <div className="col-xl-12">
              <div className="card ">
                ...
                <form className="d-flex row m-3 align-items-center">
                  <div className="col-xl-3"> <h4>WomenHelpDesk </h4></div>
                  ...
                  <div className="btn-box col-xl-3">
                    <a href="/adminwomenhelpdeskform" className="btn btn-primary"><i className="fa-light fa-plus"></i> Add New</a>
                  </div>
                </form>
                <div className="card-body admintablesroll">
                  <table className="table table-striped">
                    ...
                    <tbody>
                      <tr></tr>
                      {slicedWomenHelpDesk.map((item, index) => (
                        <tr key={index}>
                          <td>{index + 1}</td>
                          <td>{item.name}</td>
                  ...
                  <div class="modal fade" id="staticBackdrop" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true"  style={{ display: modalOpen ? 'block' : 'none' }}>
                      ...
                      <div class="modal-content">
                        <div class="modal-header">
                          <h5 class="modal-title" id="staticBackdropLabel">WomenHelpDesk </h5>
...
export default AdminWomenHelpDesk
