...
const AdminBMIForm = () => {
  ...
  useEffect(() => {
    if (bmiToUpdate) {
      setFormData({
        name: bmiToUpdate.name || '',
        nameinmarathi: bmiToUpdate.nameinmarathi || '',
        message: bmiToUpdate.message || '',
        FromDate: bmiToUpdate.FromDate || '',
        ToDate: bmiToUpdate.ToDate || '',
      });
    }
  }, [bmiToUpdate]);
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      if (bmiToUpdate) {
        // If bmiToUpdate exists, perform an update (PUT request)
        url = `http://localhost:5000/bmi/${bmiToUpdate._id}`;
        method = 'PUT';
        action = 'updated';
      } else {
        // If bmiToUpdate doesn't exist, create a new entry (POST request)
        ...
        <div className="row">
          ...
          <div className=" col-md-10 col-xl-10 col-sm-10  justify-content-center pe-0 ps-0" >
            ...
            <div className="row p-3 ">
              ...
              <a href="/adminbmi" className="text-decoration-none text-dark"> <h3 className='m-3'> <span className="material-icons-outlined pe-3 p-2">arrow_back</span>
                  BMI</h3>
                  </a>
                <hr />
