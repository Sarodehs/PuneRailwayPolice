...
const AdminIncrementList = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/incrementList/${id}`, {
        method: 'DELETE',
      });
      if (response.ok) {
  ...
  const handleUpdate = (item) => {
    navigate('/adminincrementlistform', { state: { incrementlistToUpdate: item } });
  };
  ...
  const totalPages = Math.ceil(IncrementList.length / rowsToShow);
  ...
  const slicedIncrementList = IncrementList.slice(startIndex, endIndex);
            ...
            <div className="col-xl-12">
              <div className="card ">
                ...
                <form className="d-flex row m-3 align-items-center">
                  <div className="col-xl-3"> <h4>IncrementList </h4></div>
                  ...
                  <div className="btn-box col-xl-3">
                    <a href="/adminincrementlistform" className="btn btn-primary"><i className="fa-light fa-plus"></i> Add New</a>
                  </div>
                </form>
                <div className="card-body admintablesroll">
                  <table className="table table-striped">
                    ...
                    <tbody>
                      <tr></tr>
                      {slicedIncrementList.map((item, index) => (
                        <tr key={index}>
                          <td>{index + 1}</td>
                          <td>{item.title}</td>
                  ...
                  <div class="modal fade" id="staticBackdrop" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true"  style={{ display: modalOpen ? 'block' : 'none' }}>
                      ...
                      <div class="modal-content">
                        <div class="modal-header">
                          <h5 class="modal-title" id="staticBackdropLabel">IncrementList</h5>
...
export default AdminIncrementList
