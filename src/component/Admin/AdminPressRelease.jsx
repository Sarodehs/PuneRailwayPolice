...
const AdminPressRelease = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/pressRelease/${id}`, {
        method: 'DELETE',
      });
      if (response.ok) {
        setPressRelease(PressRelease.filter(item => item._id !== id));
  ...
  const handleUpdate = (item) => {
    navigate('/adminpressreleaseform', { state: { pressreleaseToUpdate: item } });
  };
  ...
  const totalPages = Math.ceil(PressRelease.length / rowsToShow);
  ...
  const slicedPressRelease = PressRelease.slice(startIndex, endIndex);
              ...
              <div className="col-xl-12">
                <div className="card ">
                  <form className="d-flex row m-3 align-items-center">
                    <div className="col-xl-3"> <h4>PressRelease </h4></div>
                    ...
                    <div className="btn-box col-xl-3">
                      <a href="/adminpressreleaseform" className="btn btn-primary"><i className="fa-light fa-plus"></i> Add New</a>
                    </div>
                  </form>
                  <div className="card-body">
                    <table className="table table-striped">
                      ...
                      <tbody>
                        {slicedPressRelease.map((item, index) => (
                          <tr key={index}>
                            <td>{index + 1}</td>
                            <td>{item.title}</td>
                    ...
                    <div class="modal fade" id="staticBackdrop" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true"  style={{ display: modalOpen ? 'block' : 'none' }}>
                      ...
                      <div class="modal-content">
                        <div class="modal-header">
                          <h5 class="modal-title" id="staticBackdropLabel">Press Release</h5>
