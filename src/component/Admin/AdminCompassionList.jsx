...
const AdminCompassionList = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/compassionlist/${id}`, {
        method: 'DELETE',
      });
      if (response.ok) {
  ...
  const handleUpdate = (item) => {
    navigate('/admincompassionlistform', { state: { compassionlistToUpdate: item } });
  };
  ...
  const totalPages = Math.ceil(CompassionList.length / rowsToShow);
  ...
  const slicedCompassionList = CompassionList.slice(startIndex, endIndex);
            ...
            <div className="col-xl-12">
              <div className="card ">
                ...
                <form className="d-flex row m-3 align-items-center">
                  <div className="col-xl-3"> <h4>CompassionList </h4></div>
                  ...
                  <div className="btn-box col-xl-3">
                    <a href="/admincompassionlistform" className="btn btn-primary"><i className="fa-light fa-plus"></i> Add New</a>
                  </div>
                </form>
                <div className="card-body">
                  <table className="table table-striped">
                    ...
                    <tbody>
                      <tr></tr>
                      {slicedCompassionList.map((item, index) => (
                        <tr key={index}>
                          <td>{index + 1}</td>
                          <td>{item.name}</td>
...
export default AdminCompassionList
