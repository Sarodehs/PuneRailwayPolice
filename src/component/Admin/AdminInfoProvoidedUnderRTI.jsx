...
const AdminInfoProvoidedUnderRTI = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch("http://localhost:5000/infopprovoidedunderrti");
        if (response.ok) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/infopprovoidedunderrti/${id}`, {
        method: 'DELETE',
      });
      if (response.ok) {
  ...
  const handleUpdate = (item) => {
    navigate('/admininfopprovoidedunderrtiform', { state: { infopprovoidedunderrtiToUpdate: item } });
  };
  ...
  const totalPages = Math.ceil(InfoProvoidedUnderRTI.length / rowsToShow);
  ...
  const slicedInfoProvoidedUnderRTI = InfoProvoidedUnderRTI.slice(startIndex, endIndex);
            ...
            <div className="col-xl-12">
              <div className="card ">
                ...
                <form className="d-flex row m-3 align-items-center">
                  <div className="col-xl-3"> <h4>Info Provoided Under RTI </h4></div>
                  ...
                  <div className="btn-box col-xl-3">
                    <a href="/admininfopprovoidedunderrtiform" className="btn btn-primary"><i className="fa-light fa-plus"></i> Add New</a>
                  </div>
                </form>
                <div className="card-body">
                  <table className="table table-striped">
                    ...
                    <tbody>
                      <tr></tr>
                      {slicedInfoProvoidedUnderRTI.map((item, index) => (
                        <tr key={index}>
                          <td>{index + 1}</td>
                          <td>{item.name}</td>
...
export default AdminInfoProvoidedUnderRTI
