...
const AdminMedicalBills = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch("http://localhost:5000/medicalbills");
        if (response.ok) {
          const data = await response.json();
          setMedicalBills(data); // Update the state with the fetched data
        } else {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/medicalbills/${id}`, {
        method: 'DELETE',
      });
      if (response.ok) {
            ...
            <div className="col-xl-12">
              <div className="card ">
                ...
                <form className="d-flex row m-3 align-items-center">
                  <div className="col-xl-3"> <h4>MedicalBills </h4></div>
                  ...
                  <div className="btn-box col-xl-3">
                    <a href="/adminmedicalbillsform" className="btn btn-primary"><i className="fa-light fa-plus"></i> Add New</a>
                  </div>
                </form>
