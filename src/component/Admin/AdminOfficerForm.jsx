...
const AdminOfficerForm = () => {
  ...
  useEffect(() => {
    if (officerToUpdate) {
      setFormData({
        ...
        message: officerToUpdate.message || '',
        FromDate: officerToUpdate.FromDate || '',
        ToDate: officerToUpdate.ToDate || '',
      });
    }
  }, [officerToUpdate]);
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      if (officerToUpdate) {
        // If officerToUpdate exists, perform an update (PUT request)
        url = `http://localhost:5000/officer/${officerToUpdate._id}`;
        method = 'PUT';
        action = 'updated';
      } else {
        // If officerToUpdate doesn't exist, create a new entry (POST request)
        url = 'http://localhost:5000/officer';
        method = 'POST';
        action = 'created';
      }
